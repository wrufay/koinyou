...
export default async function ChapterPage({ params, searchParams }: PageProps) {
  ...
  const [data, bibles] = await Promise.all([
    fetchChapter(bookId, chapter, bibleId),
    fetchBibles(),
  ]);
    ...
    <main className="figtree min-h-screen px-5 py-12 bg-gradient-main texture-overlay relative overflow-hidden">
      ...
      <div className="max-w-lg mx-auto relative z-10">
        ...
        <div className="glass-card rounded-3xl p-8 sm:p-10 opacity-0 animate-fade-in-up stagger-1 mb-4">
          ...
          <p className="figtree-light text-xs text-olive/60 text-center mb-8">{data.verses.length} verses</p>
          ...
          <div className="flex flex-wrap gap-2">
            {data.verses.slice(0, 30).map((v) => (
              <Link
                ...
                className="figtree-medium px-3 py-1.5 rounded-full bg-white/50 hover:bg-white/80 border border-pine/20 hover:border-pine/40 text-dark-teal text-xs tag-hover"
              >
                {v.verse}
              </Link>
            ))}
            {data.verses.length > 30 && (
              <span className="figtree-light text-xs text-olive/60 self-center">+{data.verses.length - 30} more</span>
            )}
          </div>
        </div>
